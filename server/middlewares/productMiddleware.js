...
class productMiddleware {
  ...
  static async matchProductsByImages(similarities) {
    ...
    for (let similarity of similarities) {
      let product = await Product.findOne({
        imagesNames: { $in: [similarity[0]] },
      });
      products.push({ product, probability: similarity[1] });
    }
    products = products.map((element) => ({
      _id: element.product._id,
      publisherId: element.product.publisherId,
      reviews: element.product.reviews,
      name: element.product.name,
      price: element.product.price,
      description: element.product.description,
      category: element.product.category,
      imagesNames: element.product.imagesNames,
      quantities: element.product.quantities,
      createdAt:element.product.createdAt
    }));
    // products=products.map(element=>({... element.product}))
    return products;
  }
  ...
  static subtractQuantities(product, quantities) {
    for (let { size, quantity } of quantities) {
      const existingQuantityIndex = product.quantities.findIndex(
        (q) => q.size === size
      );
  ...
  static async deleteImages(product, imagesNames) {
    ...
    if (!allIncluded)
      return throwError(
        "the images you sent do not match with the products images",
        403
      );
    ...
    for (const imageName of imagesNames) {
      ...
      await access(imagePath).catch((err) =>
        throwError(`images ${imageName} was not found`, 404)
      ); // Check if the image exists
      await unlink(imagePath).catch((err) =>
        throwError(`could not delete image ${imageName}`, 400)
      ); // Delete the image

      const index = product.imagesNames.indexOf(imageName);
      if (index !== -1) product.imagesNames.splice(index, 1); // Remove image name from product.imagesNames
    }
    return product;
  }
  static async checkProductsQuantities(bill) {
    let updatedProducts = [];
    for (let item of bill.products) {
      let product = await Product.findById(item.productId);
      if (!product) {
        throwError(`Product with ID ${item.productId} was not found`, 404);
      }
      for (let quantity of product.quantities) {
        if (quantity.size === item.quantity.size) {
          if (!quantity.quantity) {
            throwError(`there is no items of this size left`, 404);
          }

          quantity.quantity = quantity.quantity - item.quantity.quantity;
          if (quantity.quantity < 0) {
            throwError(`the requested qunatity does not exist`, 404);
          }
        }
      }
  ...
  static async validateProduct_preInit(product) {
    try {
      ...
      if (
        !Array.isArray(product.imagesUrls) ||
        product.imagesUrls.some((url) => typeof url !== "string")
      ) {
      ...
      for (const { size, quantity } of product.quantities) {
        if (
          typeof size !== "string" ||
          !["XS", "S", "L", "XL", "XXL", "XXXL"].includes(size)
        ) {
